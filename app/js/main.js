...
jQuery(function($) {
	...
	pofloor.on("scrollStart", function(event, floor) {
		$("section").removeClass("selected");
		$("section:eq(" + floor.to + ")").addClass("selected");
	});
			...
			$.ajax({
				...
				success: function(data) {
					if (data.result != "success") {
						...
						$(".newsletter .message").delay(0).slideUp("fast");
						$(".newsletter .waiting").delay(0).slideDown("slow");
						$(".newsletter .waiting").delay(600).slideUp("fast");
						$(".newsletter .message").delay(1500).slideDown("slow");
						setTimeout(function() {
							$(".newsletter .message").html('<p class="error alert alert-danger"><i class="fa fa-exclamation-triangle"></i> ' + message + '</p>');
						}, 1500);
					} else {
						var message = data.msg.substring(4);
						$(".newsletter .message").delay(0).slideUp("fast");
						$("#subscribe").delay(300).slideUp("slow");
						$(".newsletter .waiting").delay(0).slideDown("slow");
						$(".newsletter .waiting").delay(900).slideUp("fast");
						$(".newsletter .message").delay(1500).slideDown("slow");
						setTimeout(function() {
							$('.newsletter .message').html('<p class="success alert alert-success"><i class="fa fa-thumbs-up"></i> ' + 'Awesome! We sent you a confirmation email.' + '</p>');
						}, 1500);
					}
				}
			});
	...
	$(".timer").potimer({
		...
		year: 2015,
		onChange: function() {
			countPOAnimate($('.timer').find('.number span'));
		}
	});

	function countPOAnimate($el) {
		$el.each(function(index) {
			...
			if (fieldText != fieldData) {
				$this.attr('data-value', fieldText).attr('data-old', fieldData).addClass('animate');
				window.setTimeout(function() {
					$this.removeClass('animate').attr('data-old', fieldText);
				}, 300);
			}
		});
	}
